use netlink_packet_netfilter::{
    conntrack::{ConntrackAttribute, IPTuple, ProtoTuple, Protocol, Tuple},
...
use netlink_packet_netfilter::{
    conntrack::ConntrackMessage, NetfilterHeader, NetfilterMessage, NetfilterProtoFamily,
};
...
// Converts a `ConntrackFlow` into a vector of `ConntrackAttribute`
// attributes suitable for a netlink message payload.
impl ConntrackFlow {
    fn to_attributes(self: ConntrackFlow) -> Vec<ConntrackAttribute> {
        ...
        if let Some(ref origin_flow) = self.origin {
            ...
            if !origin_attributes.is_empty() {
                attributes.push(ConntrackAttribute::CtaTupleOrig(origin_attributes));
            }
        }
        ...
        if let Some(ref reply_flow) = self.reply {
            ...
            if !reply_attributes.is_empty() {
                attributes.push(ConntrackAttribute::CtaTupleReply(reply_attributes));
            }
        }
            ...
            (Some(origin), Some(reply)) => {
                ...
                if origin_is_v6 != reply.is_ipv6() {
                    Err(NetavarkError::Message(
                        "Mismatched IP families in conntrack flow".to_string(),
                    ))
                } else {
                    Ok(origin_is_v6)
                }
...
pub fn parse_ct_new_msg(msg: &NetfilterMessage) -> Option<ConntrackFlow> {
    ...
    for nla in attributes {
        match nla {
            ConntrackAttribute::CtaTupleOrig(tuples) => origin_tuples = Some(tuples.as_slice()),
            ConntrackAttribute::CtaTupleReply(tuples) => reply_tuples = Some(tuples.as_slice()),
            _ => {}
        }
