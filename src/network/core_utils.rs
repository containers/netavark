...
mod tests {
    ...
    fn test_validate_subnets() {
        ...
        for tc in test_cases {
            let subnets = tc.subnets.map(|subnet_strs| {
                subnet_strs
                    .into_iter()
            ...
            let network = types::Network {
                ...
                id: "test".to_string(),
                internal: false,
                ipv6_enabled: subnets
                    .as_ref()
                    .map(|s: &Vec<types::Subnet>| {
                        s.iter().any(|subnet| subnet.subnet.addr().is_ipv6())
                    })
                    .unwrap_or(false),
                name: "test".to_string(),
            ...
            let result = validate_subnets(&network);
