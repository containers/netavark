...
impl firewall::FirewallDriver for FirewallD {
    ...
    fn setup_network(&self, network_setup: internal_types::SetupNetwork) -> NetavarkResult<()> {
        ...
        need_reload |=
            match add_policy_if_not_exist(&self.conn, POLICYNAME, ZONENAME, "ANY", "ACCEPT", true, None) {
                Ok(b) => b,
                Err(e) => {
                    return Err(NetavarkError::wrap(
        ...
        // Priority -2 is one tick higher than usual - so this executes before regular port-forwarding.
        need_reload |= match add_policy_if_not_exist(
            &self.conn,
            HOSTFWDPOLICYNAME,
            "HOST",
                        ...
                        match host_ip {
                            IpAddr::V4(_) => {
                                if let Some(ctr_ip_v4) = setup_portfw.container_ip_v4 {
                                    let rule =
                                        get_port_forwarding_hostip_rich_rule(port, &ctr_ip_v4);
                            ...
                            IpAddr::V6(_) => {
                                if let Some(ctr_ip_v6) = setup_portfw.container_ip_v6 {
                                    let rule =
                                        get_port_forwarding_hostip_rich_rule(port, &ctr_ip_v6);
                                    rich_rules.append(Value::new(rule))?;
                                }
                            }
                        }
            ...
            for rule in old_rich_rules.iter() {
                match rule {
                    Value::Str(old_rule) => {
                        ...
                        if !is_match {
                            new_rich_rules.append(rule.clone())?;
                        }
                    }
                    _ => {
                        return Err(NetavarkError::msg(
...
fn get_policy_config(
    ...
    match policy_config_msg.body::<HashMap<&str, Value>>() {
        Ok(m) => {
            for (k, v) in m {
                policy_config.insert(k.to_string(), v.into());
            }
        }
        Err(e) => {
            return Err(NetavarkError::wrap(
