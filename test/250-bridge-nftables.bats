...
net/ipv4/conf/podman0/rp_filter = 2"
   run_in_host_netns cat /run/sysctl.d/10-netavark-podman0.conf
   ...
   # The file must not be leaked. Bridge names are reused so systemd-sysctl would
   # apply forwarding = 0 to the next, possibly not internal, network with that name.
   expected_rc=1 run_in_host_netns cat /run/sysctl.d/10-netavark-podman0.conf
}

@test "$fw_driver - stale sysctl.d file is replaced on setup" {
   # Simulate a leaked file from an internal network with the same bridge name,
   # i.e. one written by an older netavark or left behind by a killed process.
   run_in_host_netns sh -c 'printf "# autogenerated by netavark\nnet/ipv4/conf/podman0/forwarding = 0\nnet/ipv4/conf/podman0/rp_filter = 2\n" > /run/sysctl.d/10-netavark-podman0.conf'
...
net/ipv4/conf/podman0/rp_filter = 2"
   run_in_host_netns cat /run/sysctl.d/10-netavark-podman0.conf
   assert "$output" == "$sysctl_content" "stale sysctl.d file content was replaced"

   run_in_host_netns cat /proc/sys/net/ipv4/conf/podman0/forwarding
   assert "$output" == "1" "forwarding enabled on the bridge"
