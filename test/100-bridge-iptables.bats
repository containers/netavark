...
@test "$fw_driver - bridge with outbound addr6" {
    ...
    # Check that the ip6tables rules were created with SNAT
    run_in_host_netns ip6tables -t nat -S NETAVARK-F11DC6A6D09CF
    assert "${lines[2]}" == "-A NETAVARK-F11DC6A6D09CF ! -d ff00::/8 -j SNAT --to-source fd20:100:200::1"

    run_netavark --file ${TESTSDIR}/testfiles/bridge-outbound-addr6.json teardown $(get_container_netns_path)

    # Check that the chain is removed
    expected_rc=1 run_in_host_netns ip6tables -t nat -nvL NETAVARK-F11DC6A6D09CF
}
