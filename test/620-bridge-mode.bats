...
@test "bridge - unmanaged mode with vlan and dhcp" {
    ...
    run_in_host_netns test -S /run/podman/nv-proxy.sock
    ...
    # The proxy uses this shared interface for renewals, so it belongs to the
    # lifetime of the unmanaged bridge rather than an individual container.
    run_in_host_netns ip link show brtest0.40

    # A later container must reuse the interface successfully.
    create_container_ns
    run_netavark --file ${TESTSDIR}/testfiles/bridge-unmanaged-vlan-dhcp.json setup $(get_container_netns_path 2)
    setup_result="$output"
    assert_json "$setup_result" ".podman.interfaces.eth0.subnets[0].ipnet" "=~" "^10\\.10\\.40\\." "reused interface should keep DHCP on vlan 40"
    run_netavark --file ${TESTSDIR}/testfiles/bridge-unmanaged-vlan-dhcp.json teardown $(get_container_netns_path 2)
}

@test "bridge - unmanaged vlan dhcp rejects wrong vlan protocol" {
    run_in_host_netns ip link add brtest0 type bridge vlan_filtering 1
    run_in_host_netns ip link set brtest0 up
    run_in_host_netns ip link add link brtest0 name brtest0.40 type vlan protocol 802.1ad id 40

    expected_rc=1 run_netavark --file ${TESTSDIR}/testfiles/bridge-unmanaged-vlan-dhcp.json setup $(get_container_netns_path)
    assert_json ".error" "VLAN interface brtest0.40 already exists but is not 802.1q VLAN 40 on bridge brtest0"
}
